...
tags: ['engineering']
draft: true
summary: 2주마다 릴리즈되는 오픈소스 개발자 문서에 버저닝을 추가해봅시다
---

## 1. 왜 Versioning이 필요한가?

오픈소스는의 프로덕트는 매우 빨리 변한다. 이런 상황에서 어떤 버전에 어떤 기능이 지원되는지를 파악하는 것은 유저 경험에 있어 매우 중요한 부분이다.
그래서 보통 왠만한 개발자 문서에서는 대부분 Versioning이 적용되어 있는 것을 볼 수있다. 다음은 [Reactive Native 문서](https://reactnative.dev/docs/getting-started)의 Versioning 예시이다.

![rn-version](/static/images/231107-versioned-docs/rn-version.png)
...
![version-warning](/static/images/231107-versioned-docs/version-warning.png)


### 3.1 복잡성의 증가

일단 Versioning을 적용하게 되면 위에서 봤듯이 아키텍쳐가 복잡해질수밖에 없다.
그에 따라서 Contribution이나 Release process도 자연스레 복잡해진다. 이미 문서가 꽤 복잡하게 운영되고 있던 터라 이 부분이 걱정이 안 될수가 없었다.
그리고 특정 버전의 문서에서 오류를 발견 했을때, 과거 버전의 문서에서도 그걸 다 고쳐야 하는가? 하는 의문도 생기게 된다.

나중에 알게 된 거지만, Multiple Sidebar를 사용하도록 바뀌거나 하면 그 전의 Version에서 새로운 Sidebar를 찾을 수 없다는 오류가 발생하는 등의 문제가 있었다.
...
그리고 기존 문서의 상당 부분이 Script로 auto generate 되는 부분이 많았는데, 이런 특성까지 합쳐져서 조금더 복잡한 부분들이 있었다.

## 3. 옵션 고려

Native Versioning을 이용하지 않고 버전 관리를 적용하는 방법도 있긴 했다. 다른 오픈소스 회사들이 Archived Versions을 관리할 때 많이 사용하는 방식인데 그냥 특정 배포의 외부 URL을 링크하는 것이다.
Docusaurus도 예전 버전에 대해 이런 방법을 사용하고 있다. `Archived Versions` 아래에 있는 버전을 클릭하면 특정 Netlify 배포로 연결된다.

![old-versions](/static/images/231107-versioned-docs/old-versions.png)

매우 단순하고 앞으로의 배포 과정을 딱히 바꾸지 않아도 된다는 장점이 있으나 다음과 같은 의견에 부딪혔다.
- 예전 버전에서, 그 다음 버전으로 이동하기가 불가능
- 비슷한 맥락으로 한 문서에서 버전간의 이동이 불가능
...
- 기존 Image 를 모두 절대 경로로 바꾼다.
- Sidebar에 버저닝이 적용되도록 수정한다.
- Version을 생성한다.
- 이 모든것에 대한 Runbook을 만든다.

### 4.1 Image 핸들링 & markdown-link-check
...
왜냐면 Docusaurus는 깨진 이미지 경로에 대해 build 과정에서 딱히 검증하지 않기 때문이다. 그러면 깨진 이미지가 실제로 배포될 위험성이 있었다.
...
### 4.3 Versioned Directory를 외부 Repository에 분리
...
│   └── version-1.0.0-sidebars.json
```
매 버전이 릴리즈 될때마다 점점 무거워지고 하나의 PR에 Changed files 300개씩 올라와서 로딩도 안되는 사태를 만들 수는 없었다.
...
요약하자면 외부 레포 존재하는 버전의 소스코드(`versioned_docs/`와 `versioned_sidebars/` 모두)를 다 긁어온뒤 특정 디렉토리에 다운로드하고 압축을 풀어 빌드할수 있게 만들어 주는 것이다.
...
이 Task가 기본적으로 로컬 테스트 및 CICD 에 둘다 적용되어야 했기 때문에 yarnStart, yarnBuild 등에 대해 dependancy 로 추가했다.
```gradle
...
결론적으로는 메인 레포에 위의 디렉토리가 포함되지 않기 때문에 메인 레포를 가볍고 직관적으로 유지할 수 있게 되었다.
또 새로운 릴리즈 시 메인 레포에 올라가는 변경 사항은 `version.json`에 추가되는 한 줄 뿐이어서 매우 직관적이다.

### 4.4 런북 만들기

실무에서 이런 버저닝 작업은 프로덕트 릴리즈와 동시에 이루어져야 한다.
그래서 이 과정을 프로덕트 릴리즈 런북에 포함했는데, 다음의 내용들이 추가되었다.
- 전체 프로덕트 릴리즈 중 어떤 순서에 이 과정이 있어야 하는지
- 새로운 버전에 관련된 문서들이 제대로 들어가 있는지 점검하는 내용

## 5. 결과 & 남아있는 과제들

결과적으로 이 체제를 바탕으로 벌써 3개의 버저닝이 배포가 되었다. 다른 Navbar section과 구분하기 위해 디자인 개선도 진행하였다.

![dh-version](/static/images/231107-versioned-docs/dh-version.png)

내부적으로도 외부적으로도 드디어 버저닝! 하는 반응이 왔다. 내적 관종은 언제나 내가 만든 결과물이 세상에 나가면 기쁘지만 여전히 다음과 같은 과제가 남아있는 것 같다.

- 프로덕트 릴리즈와 문서 작성이 정말 자연스럽게 잘 얼라인이 될려면 어떻게 개선해야 할지
- 지금은 버저닝을 생성하고 압축해서 외부 레포에 올리는 것이 수동으로 진행되고 있는데 이 과정을 어떻게 자동화할지
